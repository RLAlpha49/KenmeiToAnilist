...
export async function searchComickManga(
  ...
  try {
    ...
    const data = await window.electronAPI.comick.search(query, limit) as ComickManga[];
    ...
    console.log(`📦 Comick search found ${data?.length || 0} results for "${query}"`);
    return data || [];
  } catch (error) {
 ...
 * @source
 */
export async function getComickMangaDetail(slug: string): Promise<ComickMangaDetail | null> {
  try {
    console.log(`📖 Getting Comick manga details for: ${slug}`);
    
    // Use IPC to call the main process instead of direct fetch to avoid CORS issues
    const data = await window.electronAPI.comick.getMangaDetail(slug) as ComickMangaDetail;
    return data || null;
  } catch (error) {
...
export async function extractAniListIdFromComick(
  ...
  try {
    ...
    if (!detail?.comic) {
      console.log(`🔗 No comic data found for Comick manga: ${comickManga.title}`);
      return null;
    }
    ...
    if (!links) {
      console.log(`🔗 No external links found for Comick manga: ${comickManga.title}`);
      return null;
    }

    if (typeof links !== 'object') {
      console.log(`🔗 Links is not an object for Comick manga: ${comickManga.title}`, { linksType: typeof links, links });
      return null;
    }
    ...
    if (!anilistId) {
      console.log(`🔗 No AniList ID found for Comick manga: ${comickManga.title}`, { availableLinks: Object.keys(links) });
      return null;
    }
    ...
    if (isNaN(parsedAnilistId)) {
      console.log(`🔗 Invalid AniList ID format for Comick manga: ${comickManga.title}`, { anilistId });
      return null;
    }

    console.log(`🎯 Found AniList ID ${parsedAnilistId} for Comick manga: ${comickManga.title}`);
    
    return parsedAnilistId;
  } catch (error) {
    console.error(`❌ Failed to extract AniList ID for Comick manga ${comickManga.title}:`, error);
    return null;
  }
}
...
export async function searchComickAndGetAniListManga(
  ...
  try {
    ...
    console.log(`📦 Found ${comickResults.length} Comick results, extracting AniList IDs...`);
    ...
    console.log(`🎯 Found ${anilistIds.length} AniList IDs from Comick: [${anilistIds.join(', ')}]`);
    ...
    if (!anilistManga || anilistManga.length === 0) {
      console.log(`❌ Failed to fetch AniList manga for IDs: [${anilistIds.join(', ')}]`);
      return [];
    }
    ...
    console.log(`✅ Successfully enhanced ${enhancedManga.length} AniList manga with Comick source info`);
    return enhancedManga;
    
  } catch (error) {
    console.error(`❌ Comick search and AniList fetch failed for "${query}":`, error);
    return [];
  }
}
...
export function getComickCacheStatus() {
  ...
  const expiredEntries = Object.keys(comickCache).filter(key => 
    Date.now() - comickCache[key].timestamp > CACHE_EXPIRY
  ).length;
